...
corTab$name[abs(corTab$MBPI)>0.5] #Medium, Broad, and Fine BPI are highly correlated. Just keep Medium.
...
corTab$name[abs(corTab$fetchSum)>0.5] #Sum of fetch and minimum fetch are correlated. Just keep minimum fetch. 

#Choose ones to keep and check new correlation plot
toKeep<-c("ArcRug","bathy","fetchMin","MBPI","salt_min","Slope","Substrate","temp_meanSummer","tidalcurr_meanSummer") #9 variables
...
spEnv2<-spEnv[,!names(spEnv) %in% toDrop]
names(spEnv2)
